...
- **Tables**: light lists are white paper (not cream); dark lists use a matching lifted field.
- **Select**: one tint on the whole row — tick, Date, and Status, whether you click or tick. No second focused mix.
- **Opening balance**: that row is not selectable and no longer picks up a phantom highlight.
...
- **Dates**: every list and print date is MM/DD/YYYY (09/13/2026), zero-padded, tabular.
- **Tables**: title row stays frozen at the top; Date is not a freeze pane. Tick stays put on sideways scroll, with a gap before Date.
- **Select**: the row tint includes the tick, edge to edge, same mix as Select all.
- **Theme**: header band uses the navy/cream tokens (not a second palette).
...
- **Sidebar**: People and Planning headings stay visible so Employees and Forecast are grouped, not orphan links.
- **Tables**: last column (Status) is edge-to-edge — no reserved scrollbar strip; the title paints above the cream radius.
- **Reconcile / Register**: the tick column stays put when you scroll, with an opaque cell so it does not slide over Date.
- **Close the month**: no phantom first-row tint; short lists no longer grow their own scrollbar.
- **Customers / Vendors**: with nobody picked, the directory is one first-tab shade; a selected name is a single tint (not stacked).
...
- **Focus tint (I16)**: slightly stronger soft primary mix on focused rows (11%/20% light/dark) — still bg-only, no ring/bar

## What's new in v3.62.79

- **PartyCombo tokens (I9)**: list `z-[200]` like Select; row hits `min-h-11` + `rounded-md` to match SelectItem chrome

## What's new in v3.62.78

- **Coarse resize titles (I15)**: ColResize tip/aria on touch says double-tap to auto-fit (no “drag to resize” that cannot work)

## What's new in v3.62.77

- **Desk Filters Sort (I14)**: hide the Sort select inside Filters on `md+` — column headers already sort on desk; phone Filters sheet keeps Sort

## What's new in v3.62.76

- **ActionsHeader touch fit (I12)**: long-press matches right-click / double-click autofit (same gesture language as SortHeader align)

## What's new in v3.62.75

- **Nav single-item groups (I11)**: hide the section heading when a sidebar group has only one link (Employees, Forecast) — avoids duplicate “Employees / Employees” chrome

## What's new in v3.62.74

- **Column menu a11y (I10)**: SortHeader `aria-sort` when active; Shift+F10 / ContextMenu opens align menu from the header or title button (with right-click / long-press)

## What's new in v3.62.73

- **isPhoneUi unify (I13)**: shared `isPhoneUi()` + media query in `phone-layout.ts`; Register/Reconcile drop local copies (usePhoneUi still for reactive trees)

## What's new in v3.62.72

- **I6/I7 list CSS + colfit share**: Register and lists share `FIT_MARK`/`FIT_VERSION` + `clampCol` (Register still hydrates via UI_KEY — phone skip / check col unchanged). CSS: drop forced `.col-money` center, unify `data-align` body rules, `register-matrix` col-actions min parity

## What's new in v3.62.71

- **Default column align (I5)**: money/amount ids (`amount`, `payment`, `deposit`, `balance`, `debit`, `credit`, `total`, `rate`, `book`, `pending`, `orig`) default **right**; other cols stay center. Long-press/right-click override + saved localStorage prefs unchanged

## What's new in v3.62.70

- **Column hide API (I3)**: removed dead SortHeader `onVisible` / Hide menu (never wired). Options tip is align-only; Register **View** chips remain the real column show/hide path
...
- **Phone/touch column align (I2)**: long-press a SortHeader title to open left/center/right (same menu as right-click). No always-visible ⋮. Suppresses the sort tap that would follow a long-press. Options Gestures tip + Register View tip updated
...
- **⋮ align leftovers**: Forecast, Reports (aging/TB/P&L), Settings Recurring, Close (checklist Status + Detail/Check, snapshot, audit), Receive Payment alloc, journal lines — SortHeader `onAlign` + body `data-align` like Register
- **Phone Register list**: custom `<th>` headers replaced with SortHeader (+ ⋮ align); body cells honor left/center/right; Grid cards unchanged
- **Reconcile phone list**: same SortHeader + align treatment for Date/Payee/Amount/Days
- Cluster chrome `[ Title ↑/↓ ⋮ ]` and list density unchanged

## What's new in v3.62.62

- **SortHeader cluster**: titles stay readable on narrow cols — `[ Title ↑/↓ ⋮ ]` as one centered group (no far-right ⋮ eating width; "Bank"/"Payment" no longer clip to "B…"/"Pay…")
- **Register Status**: same header chrome as other cols (SortHeader + ⋮ align); no detached/floating Status cell
- **Column align**: Register ⋮ left/center/right now sticks on body cells (CSS vars beat default-center / money rules); Status align included
- **List density**: Comfortable/Compact change `--list-*` padding/hit targets on every list tab (Register, Reconcile, …); SortHeader height follows via CSS
...
- **Table structure**: header titles dead-center (H+V); body cells honor ⋮ align (default center) across lists + Register; one flex filler only (Register no longer stretches payee+memo); actions col hugs (`width: 1%`, `--list-actions-px`); last data col keeps resize; FIT reflow (`content-7`)
- **Select/focus**: selected and focused rows/cards use soft background tint only — no ring, border, outline, or left bar (Register, Reconcile, list tables, phone cards)

## What's new in v3.62.58

- **SortHeader (all lists)**: no colon; column **title always centered**; sort ↑/↓ sits on the far right **immediately before** ⋮ (align menu) — `[ Title ] …… [↓][⋮]`
- **Cell align default = center** for every column (prior money-right / text-left defaults removed). Saved ⋮ prefs still win. FIT bumped so widths reflow once
- Shared via SortHeader / list-grid — Banks, Checks, Receipts, Invoices, Bills, Customers, Vendors, Employees, Ledger, Reconcile, Register
...
- **Column align options**: header ⋮ menu (or right-click) sets left/center/right per column; persisted. Defaults: center for all columns (⋮ overrides; prefs persist). Register View still toggles columns on/off
