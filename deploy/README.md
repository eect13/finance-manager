...
A fresh GitHub zip (including `finance-manager-main (1)`) is enough — double-click the one-click at the unzip root. `node_modules` is not in git; the scripts run `npm install`.
...
| **Android** | `apk.bat` or `deploy/android/apk.bat` | Real **Tauri APK** (not a PWA). Needs JDK 17 + Android Studio SDK+NDK. Output in `deploy/android/`. |
