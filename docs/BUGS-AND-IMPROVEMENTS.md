...
| # | Severity | Issue | Fix |
| --- | --- | --- | --- |
| S1 | Med | Country pack Apply was enable-only (US left PH on) | `modulesEnabledByPack` / `settingsPatchForCountryPack` set pack modules on and turn other region packs off; genericVat intentional per pack; multiCurrency preserved |
| S2 | Low | Close Detail left default lost to stored center | Bump `finance-manager-close-check-col-aligns` → `…-v2`; Detail defaults left, Status center |
| S3 | Low | Print payroll / BIR disclaimer when modules off | Hide Print on Payroll tab unless PH payroll; VAT panel disclaimer gated to BIR / genericVat |
...
5. **Payroll depth** — Statutory PH engine (v3.63.17) + 13th-month / 1601-C CSV (v3.63.37) + regional module toggles (v3.63.38). US/SG/AU/UK/VAT/FX stubs shipped (v3.63.40). Still open: full BIR annualization / TRAIN year-end / eBIRForms XML; IRS Pub 15-T / W-2 e-file; CPF Board; ATO BAS/STP; HMRC RTI; per-invoice FX + realized gain/loss journals.
