#!/usr/bin/env node
/**
 * One-shot screenshot refresh for README docs/screenshots (v3.62.4).
...
// Options → Currency and tax
  ...
  if (await postBtn.count()) {
    ...
    for (let i = 0; i < Math.min(n, 8); i++) {
      ...
      if (ph.includes("payee") || aria.includes("payee") || ph.includes("name")) {
        ...
        await el.fill("v3.62.4 screenshot").catch(() => null);
      }
    }
  ...
  await p2.goto(BASE + "/customers", { waitUntil: "networkidle", timeout: 120_000 });
  await p2.waitForTimeout(1200);
  await p2.screenshot({ path: join(OUT, "customers-phone.png"), fullPage: false });
  console.log("wrote customers-phone.png");

  await p2.goto(BASE + "/settings", { waitUntil: "networkidle", timeout: 120_000 });
  await p2.waitForTimeout(800);
  const zoomLabel = p2.locator("text=Display zoom").first();
  await zoomLabel.scrollIntoViewIfNeeded().catch(() => null);
  await p2.waitForTimeout(400);
  await p2.screenshot({ path: join(OUT, "options-display-zoom-phone.png"), fullPage: false });
  console.log("wrote options-display-zoom-phone.png");
